...
/** Server-authoritative Green Terra preparation, duration, and persisted cooldown. */
public final class HerbalismAbilityController implements AutoCloseable {
    public static final long PREPARATION_WINDOW_MILLIS = 4_000L;
    private final HerbalismAbilityStore store;
    private final Clock clock;
    private final Map<UUID, RuntimeState> states = new HashMap<>();

    public HerbalismAbilityController(HerbalismAbilityStore store, Clock clock) {
    ...
    public synchronized Activation activate(
        ...
        int durationSeconds = settings.greenTerraDurationSeconds(skillLevel)
                + Math.max(0, activationBonusSeconds);
        state.activeUntil = now + durationSeconds * 1_000L;
        // Upstream stores the deactivation timestamp for Green Terra.
        state.persisted = new HerbalismAbilityData(state.activeUntil);
    ...
    public synchronized void reset(UUID playerId) throws IOException {
        RuntimeState state = state(playerId);
        state.persisted = HerbalismAbilityData.EMPTY;
        state.preparedUntil = 0L;
    ...
    private static final class RuntimeState {
        private HerbalismAbilityData persisted;
        private long preparedUntil;
        private long activeUntil;

        private RuntimeState(HerbalismAbilityData persisted) {
    ...
    public sealed interface Preparation {
        ...
        Preparation ALREADY_PREPARED = new Simple("already_prepared");
