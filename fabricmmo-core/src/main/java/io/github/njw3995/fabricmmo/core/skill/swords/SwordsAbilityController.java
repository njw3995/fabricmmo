...
/** Server-authoritative Serrated Strikes preparation, duration, and persisted cooldown. */
public final class SwordsAbilityController implements AutoCloseable {
    public static final long PREPARATION_WINDOW_MILLIS = 4_000L;
    private final SwordsAbilityStore store;
    private final Clock clock;
    private final Map<UUID, RuntimeState> states = new HashMap<>();

    public SwordsAbilityController(SwordsAbilityStore store, Clock clock) {
        this.store = Objects.requireNonNull(store, "store");
        this.clock = Objects.requireNonNull(clock, "clock");
    }

    public synchronized Preparation prepare(
            UUID playerId, int level, SwordsSettings settings, int cooldownSeconds)
            throws IOException {
        ...
        int levelsRequired = settings.serratedUnlockLevel() - level;
        if (levelsRequired > 0) return new Preparation.Locked(levelsRequired);
        long remaining = remaining(state.persisted.serratedStrikesLastUsed(), cooldownSeconds, now);
    ...
    public synchronized Activation activate(
        ...
        state.preparedUntil = 0L;
        int duration = settings.serratedDurationSeconds(level) + Math.max(0, activationBonusSeconds);
        state.activeUntil = now + duration * 1_000L;
        // Upstream stores the ability deactivation timestamp for cooldown calculations.
        state.persisted = new SwordsAbilityData(state.activeUntil);
    ...
    public synchronized int cooldownRemaining(UUID playerId, int cooldownSeconds) throws IOException {
        ...
        return seconds(remaining(state.persisted.serratedStrikesLastUsed(), cooldownSeconds, now));
    }
    ...
    public synchronized void reset(UUID playerId) throws IOException {
        RuntimeState state = state(playerId);
        state.persisted = SwordsAbilityData.EMPTY;
        state.preparedUntil = 0L;
    ...
    private static final class RuntimeState {
        private SwordsAbilityData persisted;
        private long preparedUntil;
        private long activeUntil;
        private RuntimeState(SwordsAbilityData persisted) { this.persisted = persisted; }
    }
