...
/** Server-authoritative Skull Splitter preparation, duration, and persisted cooldown. */
public final class AxesAbilityController implements AutoCloseable {
    public static final long PREPARATION_WINDOW_MILLIS = 4_000L;
    private final AxesAbilityStore store;
    private final Clock clock;
    private final Map<UUID, RuntimeState> states = new HashMap<>();

    public AxesAbilityController(AxesAbilityStore store, Clock clock) {
        this.store = Objects.requireNonNull(store, "store");
        this.clock = Objects.requireNonNull(clock, "clock");
    }

    public synchronized Preparation prepare(
            UUID playerId, int level, AxesSettings settings, int cooldownSeconds)
            throws IOException {
        ...
        int levelsRequired = settings.skullSplitterUnlockLevel() - level;
        if (levelsRequired > 0) return new Preparation.Locked(levelsRequired);
        long remaining = remaining(state.persisted.skullSplitterLastUsed(), cooldownSeconds, now);
    ...
    public synchronized Activation activate(
        ...
        state.preparedUntil = 0L;
        int duration = settings.skullSplitterDurationSeconds(level) + Math.max(0, activationBonusSeconds);
        state.activeUntil = now + duration * 1_000L;
        // Upstream stores the ability deactivation timestamp for cooldown calculations.
        state.persisted = new AxesAbilityData(state.activeUntil);
    ...
    public synchronized int cooldownRemaining(UUID playerId, int cooldownSeconds) throws IOException {
        ...
        return seconds(remaining(state.persisted.skullSplitterLastUsed(), cooldownSeconds, now));
    }
    ...
    public synchronized void reset(UUID playerId) throws IOException {
        RuntimeState state = state(playerId);
        state.persisted = AxesAbilityData.EMPTY;
        state.preparedUntil = 0L;
    ...
    private static final class RuntimeState {
        private AxesAbilityData persisted;
        private long preparedUntil;
        private long activeUntil;
        private RuntimeState(AxesAbilityData persisted) { this.persisted = persisted; }
    }
